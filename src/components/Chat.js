    ...
    <>
      {data.chatId !== "null" ? (
        <div className="relative flex flex-col items-center min-h-screen w-full bg-shadywhite overflow-x-hidden sm:w-2/3 lg:w-3/4 h-full">
          ...
          {showPopup && (
            <div className="flex absolute top-1/3 left-2/5 ml-12 flex-col z-50 min-h-[30%] p-5 min-w-[75%] max-w-[60%] items-center text-center bg-shadyblue sm:p-7 sm:min-w-[70%] sm:max-h-[50%] sm:min-h-max md:ml-20 lg:ml-52 lg:min-w-[30%] lg:max-w-[50%] xl:ml-56">
              <div className="flex flex-col">
                <h3 className="text-white text-2xl flex justify-start md:text-2xl">{(selectedOption)}?</h3>
                <p className=" mt-4 text-gray-300 text-lg flex justify-start text-start md:text-xl">{`Do you really want to ${selectedOption}. Messages will be deleted permanently for everyone and cannot be recovered later.`} </p>
              </div>
              <div className="flex mt-3 justify-end items-end">
              <button className="ml-4 bg-transparent px-4 py-1.5 border-2 border-shadyblue rounded-xl text-gray-300 my-2.5 hover:text-blue-400 sm:ml-5 mr-3" onClick={() => setShowPopup(false)}>Cancel</button>
              <button className="ml-4 bg-blue-500 px-4 py-2 rounded-xl text-white my-2.5 hover:bg-blue-700 sm:ml-5 mr-3" onClick={handleSelectedOption}>Confirm</button>
              </div>
            </div>
          )}
            <Messages />
